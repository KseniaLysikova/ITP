...
def week_command(message):
    cursor.execute("SELECT curr_week FROM telegram_bot_info")
...
def get_schedule(next_week):
    ...
    cursor.execute("SELECT tt.day_name, s.subject_name as subject, tt.room_numb, tt.start_time as start_time, "
                   "tr.full_name as teacher FROM subject s, timetable tt, teacher tr WHERE s.subject_id = tr.subject "
                   "AND s.subject_id = tt.subject AND tt.week = " + ("((SELECT curr_week FROM telegram_bot_info) + 1)" if next_week else "(SELECT curr_week FROM telegram_bot_info)") + " order by tt.id")
